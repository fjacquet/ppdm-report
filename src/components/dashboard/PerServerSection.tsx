...
export function PerServerSection({ servers, dark }: PerServerSectionProps) {
  ...
  // Header choice is per-table, not per-row: a mixed estate with ANY sized
  // server keeps the TB column (and its per-row formatGbOrUnknown fallback).
  const anySizes = servers.some((s) => hasGapSizes(s.view.gaps))
